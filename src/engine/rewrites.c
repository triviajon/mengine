...
RewriteProof *rewrites_head(Context *goal_context, Expression *expr, int n, va_list lemmas) {
  RewriteProof *current_proof = init_rewrite_proof(expr, expr, build_eq_refl(expr), dll_create());
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (instantiated_lemma != NULL) {
        ...
        if (expr_match(lhs, current_expr)) {
          RewriteProof *curr_to_next = init_rewrite_proof(current_expr, rhs, instantiated_lemma, unification_result->new_goals);
          Expression *proof_of_curr_to_next = build_eq_trans(current_proof, curr_to_next);
          current_proof = init_rewrite_proof(expr, rhs, proof_of_curr_to_next, dll_merge(current_proof->remaining_goals, curr_to_next->remaining_goals));
          current_expr = rhs;
...
RewriteProof *rewrites_lambda(Context *goal_context, Expression *expr, int n, va_list lemmas) {
  Expression *x = expr->value.lambda.bound_variable;
  Expression *T = get_expression_type(x);
  Expression *inner_orig = expr->value.lambda.body;
...
RewriteProof *rewrites_app(Context *goal_context, Expression *expr, int n, va_list lemmas) {
  Expression *func = expr->value.app.func;
  Expression *arg = expr->value.app.arg;
  ...
  if (nothing_rewritten(rw_func_proof) && nothing_rewritten(rw_arg_proof)) {
    mid_rewrite_proof = init_rewrite_proof(expr, expr, build_eq_refl(expr), dll_create());
  } else {
    DoublyLinkedList *merged = dll_merge(rw_func_proof->remaining_goals,
                                         rw_arg_proof->remaining_goals);
    mid_rewrite_proof =
        init_rewrite_proof(expr,
                           init_app_expression(rw_func_proof->rewritten_expr, rw_arg_proof->rewritten_expr),
                           build_app_cong(rw_func_proof, rw_arg_proof), merged);
  }
...
RewriteProof *rewrites_var(Context *goal_context, Expression *expr, int n, va_list lemmas) {
...
RewrittenGoal *rewrites_transform(Expression *goal, int n, ...) {
  ...
  while (true) {
    ...
    RewriteProof *rewrite_proof = rewrites(get_expression_context(goal), curr_goal_ty, n, args);
