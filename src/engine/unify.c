...
Expression *instantiate_lemma_type(Context *context, Expression *lemma_ty) {
  switch (lemma_ty->type) {
    case (FORALL_EXPRESSION): {
      ...
      Expression *hole = init_hole_expression(bound_var->value.var.name, bound_var_ty, context);
...
Expression *_unify(Expression *exprA, Expression *exprB, Expression *hole_to_fill) {
  switch (exprA->type) {
    ...
    case APP_EXPRESSION: {
      ...
      Expression *new_arg = _unify(exprA->value.app.arg, exprB->value.app.arg, hole_to_fill);
      if (new_arg != NULL) {
        return new_arg;
      }
...
/**
 * Tries to unify a given lemma with an expression by instantiating the universally quantified 
 * variables and attempting to find appropriate substitutions. Returns a list of any remaining 
...
UnificationResult *eunify2(Expression *lemma, Expression *goal) {
  ...
  while (current_lemma_app_ty->type == FORALL_EXPRESSION) {
    Expression *bound_variable = current_lemma_app_ty->value.forall.bound_variable;
    Expression *hole_subst = _unify2(get_innermost_body(current_lemma_app_ty), expr, bound_variable);
