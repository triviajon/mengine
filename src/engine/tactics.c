...
Expression *_build_reflexivity_proof(Expression *expr, Context *ctx) {
    ...
    Expression *proof = init_app_expression_wc(init_app_expression_wc(Eq_refl, relation_over, ctx), expr, ctx);

    return proof;
}

Expression *_build_transitivity_proof(RewriteResult *first_rwr, RewriteResult *second_rwr, Context *ctx) {
    ...
    // build the term "Eq_trans relation_over original mid rewritten H1 H2"
...
Expression *_build_app_congruence_proof(RewriteResult *func_rwr, RewriteResult *arg_rwr, Context *ctx) {
    // Bad_App_Congruence : forall (A B : Type) (f g : A -> B) (x y: A), Eq (A -> B) f g -> Eq (A) x y -> Eq (B) (f x) (g y).
    // if func_rw provides pf : Eq (A -> B) f g and arg_rwr provided pg : Eq A x y,
    // build the term "Bad_App_Congruence A B f g x y pf pg"

    Expression *A_implies_B = get_app_arg(get_app_func(get_app_func(get_expression_type(func_rwr->original_to_rewritten_proof))));
    ...
    Expression *proof = init_app_expression_wc(init_app_expression_wc(init_app_expression_wc(
        init_app_expression_wc(
            init_app_expression_wc(
                init_app_expression_wc(
                    init_app_expression_wc(
                        init_app_expression_wc(
                            Bad_App_Congruence, A, ctx),
                        B, ctx),
                    f, ctx),
                g, ctx),
            x, ctx),
...
RewriteResult *init_rewrite_result(Expression *original, Expression *rewritten, DoublyLinkedList *new_goals, Expression *original_to_rewritten_proof) {
...
RewriteResult *n_rewrite_head(Expression *mid, Expression *lemma, Context *context) {
    ...
    // We're creating a hole with expected return type `mid` and defining context `context`,
    // and attempting to apply the lemma to it.
    // Expression *hole = init_hole_expression("Rewrite_Hole", mid, context);

    // // TODO: Temporary, until the hole fill bug is fixed.
    // Expression *holder = init_lambda_expression_wc(init_var_expression_wc("t", init_type_expression(), context),
    //      hole, context);
    ...
    // For rewriting, we do not allow the unification result to have any remaining open holes.
    if (dll_len(unif_result->new_goals) > 0) {
        free_unification_result(unif_result);
        return init_rewrite_result(mid, mid, dll_create(), _build_reflexivity_proof(mid, context));
    }
...
RewriteResult *n_rewrite_app(Expression *expr, Expression *lemma, Context *context) {
    ...
    if (n_rewrite_is_noop(rwr_func) && n_rewrite_is_noop(rwr_arg)) {
        mid_rwr = init_rewrite_result(expr, expr, dll_create(), _build_reflexivity_proof(expr, context));
    } else {
        mid_rwr = init_rewrite_result(
            expr,
            init_app_expression(rwr_func->rewritten, rwr_arg->rewritten),
...
TacticResult *rewrite_tactic(Expression *goal, Expression *lemma) {
    // Assume return_type has form R lhs rhs, so that R is the relation and the type of lhs/rhs are what the relation are over. 
    ...
    if (get_app_func(relation) != Eq) {
        return init_tactic_result(false, NULL, "Currently only rewriting for Leibniz Equality is supported");
    }

    // Once that's confirmed, we can begin attempting to rewrite. Start with the lhs and try to apply the lemma. 
    RewriteResult *rwr = n_rewrite(relation_left_hand, lemma, operating_ctx);
    if (!rwr) {
        return init_tactic_result(false, NULL, "Rewriting failed");
    }

    // rwr gives us Eq A lhs lhs' with proof pf. We now need to build the proof of Eq relation_over lhs rhs.
    // We'll just use Eq_trans : (forall (A: Type), (forall (x: A), (forall (y: A), (forall (z: A), (forall (_: (((Eq A) x) y)), (forall (_: (((Eq A) y) z)), (((Eq A) x) z)))))))
    Expression *new_goal_type = init_app_expression_wc(
        init_app_expression_wc(
            init_app_expression_wc(
                Eq, relation_over, operating_ctx),
    ...
    Expression *proof_of_goal = init_app_expression_wc(init_app_expression_wc(init_app_expression_wc(init_app_expression_wc(init_app_expression_wc(init_app_expression_wc(
        Eq_trans, relation_over, operating_ctx), 
