...
int context_size(Context *context) { return context->length; }
...
DoublyLinkedList *context_ancestors_until(Context *context_A,
                                          Expression *until) {
    DoublyLinkedList *list = dll_create();
    Context *curr_context = context_A;
...
Context *context_minus(Context *context, Expression *subtrahend) {
    ...
    DoublyLinkedList *given_ancestors =
        context_ancestors_until(context, subtrahend);
