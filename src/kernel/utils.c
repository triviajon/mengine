...
char *stringify_expression(Expression *expression) {
  ...
  switch (expression->type) {
    ...
    case FIX_EXPRESSION: {
      ...
      char *var_str = stringify_expression(expression->value.fix.bound_variable);
      char *type_str = stringify_expression(expression->value.fix.bound_variable->value.var.type);
    ...
    case MATCH_EXPR_EXPRESSION: {
      char *match_scrutinee_str = stringify_expression(expression->value.matchExpr.match_scrutinee);
      char *literal_case_item_str = stringify_expression(expression->value.matchExpr.literal_case_item);
      char *literal_result_str    = stringify_expression(expression->value.matchExpr.literal_result);
      char *var_case_item_str = stringify_expression(expression->value.matchExpr.var_case_item);
      char *var_result_str    = stringify_expression(expression->value.matchExpr.var_result);
      char *op_case_item_str = stringify_expression(expression->value.matchExpr.op_case_item);
      char *op_result_str = stringify_expression(expression->value.matchExpr.op_result);
...
char *stringify_expression2(Expression *expression) {
  ...
  switch (expression->type) {
    ...
    case HOLE_EXPRESSION:
      ...
      case FIX_EXPRESSION: {
        char *ident_str = stringify_expression2(expression->value.fix.ident);
        char *var_str = stringify_expression2(expression->value.fix.bound_variable);
        char *type_str = stringify_expression2(expression->value.fix.bound_variable->value.var.type);
        char *body_str = stringify_expression2(expression->value.fix.body);
        result = str_concat("(fix ", ident_str);
        result = str_concat(result, " (");
        result = str_concat(result, var_str);
        result = str_concat(result, ": ");
      ...
      case MATCH_EXPR_EXPRESSION: {
        char *match_scrutinee_str = stringify_expression2(expression->value.matchExpr.match_scrutinee);
        char *literal_case_item_str = stringify_expression2(expression->value.matchExpr.literal_case_item);
        char *literal_result_str    = stringify_expression2(expression->value.matchExpr.literal_result);
        char *var_case_item_str = stringify_expression2(expression->value.matchExpr.var_case_item);
        char *var_result_str    = stringify_expression2(expression->value.matchExpr.var_result);
        char *op_case_item_str = stringify_expression2(expression->value.matchExpr.op_case_item);
        char *op_result_str = stringify_expression2(expression->value.matchExpr.op_result);
        result = str_concat("match ", match_scrutinee_str);
        result = str_concat(result, " with ");
