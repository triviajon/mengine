...
char *_stringify_type(Expression *expr) {
    ...
    switch (expr->type) {
        case FORALL_EXPRESSION: {
            ...
            result = str_concat(result, type_str);
            result = str_concat(result, ")");
            char *rest = _stringify_type(
                expr->value.forall.body);  // Assume non-variadic
...
char *stringify_expression2(Expression *expression) {
    ...
    switch (expression->type) {
        ...
        case LAMBDA_EXPRESSION: {
            char *var_str =
                stringify_expression2(expression->value.lambda.bound_variable);
            char *type_str = stringify_expression2(
                expression->value.lambda.bound_variable->value.var.type);
            char *body_str =
                stringify_expression2(expression->value.lambda.body);
            result = str_concat("fun (", var_str);
            result = str_concat(result, ": ");
            result = str_concat(result, type_str);
            result = str_concat(result, ") => ");
        ...
        case FORALL_EXPRESSION: {
            char *var_str =
                stringify_expression2(expression->value.forall.bound_variable);
            char *type_str = stringify_expression2(
                expression->value.forall.bound_variable->value.var.type);
            char *body_str =
                stringify_expression2(expression->value.forall.body);
            result = str_concat("forall (", var_str);
            result = str_concat(result, ": ");
            result = str_concat(result, type_str);
            result = str_concat(result, "), ");
        ...
        case FIX_EXPRESSION: {
            char *ident_str =
                stringify_expression2(expression->value.fix.ident);
            char *var_str =
                stringify_expression2(expression->value.fix.bound_variable);
            char *type_str = stringify_expression2(
                expression->value.fix.bound_variable->value.var.type);
        ...
        case MATCH_EXPR_EXPRESSION: {
            char *match_scrutinee_str = stringify_expression2(
                expression->value.matchExpr.match_scrutinee);
            char *literal_case_item_str = stringify_expression2(
                expression->value.matchExpr.literal_case_item);
            char *literal_result_str = stringify_expression2(
                expression->value.matchExpr.literal_result);
            char *var_case_item_str = stringify_expression2(
                expression->value.matchExpr.var_case_item);
            char *var_result_str =
                stringify_expression2(expression->value.matchExpr.var_result);
            char *op_case_item_str =
                stringify_expression2(expression->value.matchExpr.op_case_item);
            char *op_result_str =
                stringify_expression2(expression->value.matchExpr.op_result);
            result = str_concat("match ", match_scrutinee_str);
            result = str_concat(result, " with ");
...
char *stringify_context2(Context *context) {
    if (context_is_empty(context)) {
        ...
        char *type_str =
            stringify_expression2(context->var_type->value.var.type);
    ...
    switch (expression->type) {
        ...
        case LAMBDA_EXPRESSION: {
            if (dll_len(get_expression_uplinks(expression)) > 1) {
                ...
                char *var_str = _stringify_expression_with_let2(
                    expression->value.lambda.bound_variable);
                char *type_str = _stringify_expression_with_let2(
                    expression->value.lambda.bound_variable->value.var.type);
                char *body_str = _stringify_expression_with_let2(
        ...
        case APP_EXPRESSION: {
            if (dll_len(get_expression_uplinks(expression)) > 1) {
                ...
                char *func_str =
                    _stringify_expression_with_let2(expression->value.app.func);
                char *arg_str =
                    _stringify_expression_with_let2(expression->value.app.arg);
    ...
    switch (expression->type) {
        ...
        case LAMBDA_EXPRESSION: {
            ...
            char *type_str = _stringify_expression_with_let2(
                expression->value.lambda.bound_variable->value.var.type);
            char *body_str =
                _stringify_expression_with_let2(expression->value.lambda.body);
            result = str_concat("fun (", var_str);
            result = str_concat(result, ": ");
            result = str_concat(result, type_str);
            result = str_concat(result, ") => ");
        ...
        case FORALL_EXPRESSION: {
            ...
            char *type_str = _stringify_expression_with_let2(
                expression->value.forall.bound_variable->value.var.type);
            char *body_str =
                _stringify_expression_with_let2(expression->value.forall.body);
            result = str_concat("forall (", var_str);
            result = str_concat(result, ": ");
            result = str_concat(result, type_str);
            result = str_concat(result, "), ");
...
char *stringify_expression_with_let2(Expression *expression) {
    ...
    for (int i = dll_len(ordering) - 1; i >= 0; i--) {
        ...
        if (node_expr->type != VAR_EXPRESSION &&
            ...
            char *str_adr = _get_str_addr(node_expr);
            char *expr_string =
                _top_level_stringify_expression_with_let2(node_expr);
