...
Expression *subst(Expression *expression, Expression *old_e, Expression *new_e) {
  ...
  switch (expression->type) {
    ...
    case (LAMBDA_EXPRESSION): {
      ...
      if (context_find(get_expression_context(lambda_var_ty), old_e) == NULL) {
    ...
    case (APP_EXPRESSION): {
      ...
      Expression *new_app_func = (context_find(get_expression_context(app_func), old_e) == NULL) ? app_func : subst(app_func, old_e, new_e);
      Expression *new_app_arg = (context_find(get_expression_context(app_arg), old_e) == NULL) ? app_arg : subst(app_arg, old_e, new_e);
    ...
    case (FORALL_EXPRESSION): {
      ...
      if (context_find(get_expression_context(forall_var_ty), old_e) == NULL) {
    ...
    case (FIX_EXPRESSION): {
      ...
      Expression *new_fix_var = init_var_expression(fix_var->value.var.name, new_fix_var_type);
      ...
      Expression *new_fix_ident = init_var_expression(fix_ident->value.var.name, new_fix_ident_type);
    ...
    case (MATCH_EXPR_EXPRESSION): {
      Expression *new_match_scrutinee = expression->value.matchExpr.match_scrutinee;
      Expression *new_literal_case_item = expression->value.matchExpr.literal_case_item;
      Expression *new_literal_result = subst(expression->value.matchExpr.literal_result, old_e, new_e);
      Expression *new_var_case_item = expression->value.matchExpr.var_case_item;
      Expression *new_var_result = subst(expression->value.matchExpr.var_result, old_e, new_e);
      Expression *new_op_case_item = expression->value.matchExpr.op_case_item;
      Expression *new_op_result = subst(expression->value.matchExpr.op_result, old_e, new_e);
      Expression *new_type = subst(expression->value.matchExpr.type, old_e, new_e); 

      return init_match_expr_expression(new_match_scrutinee, new_literal_case_item, new_literal_result, 
        new_var_case_item, new_var_result, new_op_case_item, new_op_result, new_type);
    }
  ...
  switch (expression->type) {
    ...
    case (FORALL_EXPRESSION): {
      ...
      if (search_result != -1) {
        ...
        return init_forall_expression(new_forall_var, p_subst(forall_body, old_exprs, new_exprs));
      }
      Expression *forall_var_ty = get_expression_type(forall_var);
    ...
    case (FIX_EXPRESSION): {
      Expression *fix_var = expression->value.fix.bound_variable;
      Expression *fix_var_ty = get_expression_type(fix_var);
      Expression *new_fix_var_type = p_subst(fix_var_ty, old_exprs, new_exprs);
      Expression *new_fix_var = init_var_expression(fix_var->value.var.name, new_fix_var_type);

      Expression *fix_ident = expression->value.fix.ident;
      Expression *fix_ident_ty = get_expression_type(fix_ident);
      Expression *new_fix_ident_type = p_subst(fix_ident_ty, old_exprs, new_exprs);
      Expression *new_fix_ident = init_var_expression(fix_ident->value.var.name, new_fix_ident_type);
    ...
    case (MATCH_EXPR_EXPRESSION): {
      Expression *new_match_scrutinee = expression->value.matchExpr.match_scrutinee;
      Expression *new_literal_case_item = expression->value.matchExpr.literal_case_item;
      Expression *new_literal_result = p_subst(expression->value.matchExpr.literal_result, old_exprs, new_exprs);
      Expression *new_var_case_item = expression->value.matchExpr.var_case_item;
      Expression *new_var_result = p_subst(expression->value.matchExpr.var_result, old_exprs, new_exprs);
      Expression *new_op_case_item = expression->value.matchExpr.op_case_item;
      Expression *new_op_result = p_subst(expression->value.matchExpr.op_result, old_exprs, new_exprs);
      Expression *new_type = p_subst(expression->value.matchExpr.type, old_exprs, new_exprs); 
