...
static MapEntry *map_entries_new(size_t capacity) {
    return calloc(capacity, sizeof(MapEntry));
}

// Size+tombstones together determine slot pressure.
static bool map_should_grow(Map *m) {
    return (m->size + m->tombstones + 1) * MAP_LOAD_FACTOR_DEN >
...
static bool map_resize(Map *m, size_t new_capacity) {
    ...
    for (size_t i = 0; i < old_capacity; i++) {
        ...
        if (e->in_use != 1) continue;  // skip empty and tombstones
...
void *map_get(Map *m, void *key) {
    ...
    while (true) {
        ...
        if (e->in_use == 0) return NULL;       // empty: stop
...
bool map_set(Map *m, void *key, void *value) {
    ...
    if (map_should_grow(m)) {
        if (!map_resize(m, m->capacity * 2)) return false;
    }
    uint32_t hash = map_hash_key(key);
    size_t idx = hash & (m->capacity - 1);
    ...
    while (true) {
        ...
        if (e->in_use == 0) {
            ...
            slot->in_use = 1;
            m->size++;
            if (tombstone_idx != (size_t)-1) m->tombstones--;
            return true;
        }
        if (e->in_use == 2) {
            // Tombstone: remember first one, keep probing for existing key.
            if (tombstone_idx == (size_t)-1) tombstone_idx = idx;
...
bool map_del(Map *m, void *key) {
    ...
    while (true) {
        MapEntry *e = &m->entries[idx];
        if (e->in_use == 0) return false;  // empty: not found
        if (e->in_use == 2) {              // tombstone: skip
            idx = (idx + 1) & (m->capacity - 1);
            continue;
        }
...
void map_reset(Map *m) {
    if (!m || (m->size == 0 && m->tombstones == 0)) return;
...
void map_clear_free_values(Map *m) {
    ...
    for (size_t i = 0; i < m->capacity; i++) {
        if (m->entries[i].in_use == 1) free(m->entries[i].value);
    }
    map_free(m);
}
...
void map_clear_apply_free(Map *m, void (*free_fn)(void *)) {
    if (!m) return;
    for (size_t i = 0; i < m->capacity; i++) {
        if (m->entries[i].in_use == 1) free_fn(m->entries[i].value);
    }
    map_free(m);
}
...
void map_for_each(Map *m, void (*fn)(void *key, void *value, void *ud), void *ud) {
    if (!m) return;
    for (size_t i = 0; i < m->capacity; i++) {
        if (m->entries[i].in_use == 1) fn(m->entries[i].key, m->entries[i].value, ud);
    }
}
