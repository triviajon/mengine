...
static void _handle_definition_command(MEngineRuntime *rt,
    ...
    Expression *body =
        _create_definition_body(rt, params, param_count, defn_cmd->body,
    ...
    Expression *expected_type_def_body =
        ast_to_expression(defn_cmd->type, rendered_type_ctx);
    if (!expected_type_def_body) {
        fprintf(stderr,
                ERROR "Error:" CRESET " definition '%s' has a invalid type.\n",
                name);
    }

    if (!congruence(inferred_type_def_body, expected_type_def_body)) {
        fprintf(stderr,
                ERROR "Type error:" CRESET
    ...
    Expression *defn_var =
        init_var_expression_wc_with_definition(defn_cmd->name, body, rt->ctx);
...
static void _handle_statement_command(MEngineRuntime *rt,
    ...
    if (!statement_type) {
        fprintf(stderr,
                ERROR "Failed to convert type for Statement %s\n" CRESET,
                stmt_cmd->name);
        return;
    }
...
static void _handle_check_command(MEngineRuntime *rt, CheckCmd *check_cmd) {
    ...
    if (!expr) {
        fprintf(stderr,
                ERROR "Failed to convert term in Check command.\n" CRESET);
        return;
    }
...
static Expression *_build_constructor_case_type(
    Expression *ctor_expr, Expression *ctor_type, Expression *motive_var,
    Expression **param_vars, size_t param_count, size_t index_count,
    Context *elim_ctx);
...
static Expression *_build_motive_type(Expression *ind_var,
    ...
    for (size_t i = 0; i < param_count; i++) {
        ind_with_indices =
            init_app_expression_wc(ind_with_indices, param_vars[i], motive_ctx);
    }
    for (size_t i = 0; i < index_count; i++) {
        ind_with_indices =
            init_app_expression_wc(ind_with_indices, index_vars[i], motive_ctx);
    }
...
static Expression *_build_induction_principle_type(InductiveCmd *ind_cmd,
    ...
    size_t index_count = 0;
    Expression *motive_type = _build_motive_type(
        ind_var, param_vars, param_count, elim_ctx, &index_vars, &index_count);
    ...
    for (size_t i = 0; i < ctor_count; i++) {
        ...
        if (!ctor_expr) {
            fprintf(stderr,
                    ERROR "Constructor %s not found in context\n" CRESET,
                    ctor->name);
            free(case_vars);
            free(case_contexts);
            return NULL;
        ...
        Expression *case_type = _build_constructor_case_type(
            ctor_expr, ctor_type, motive_var, param_vars, param_count,
        ...
        if (!case_vars[i]) {
            fprintf(stderr,
                    ERROR "Failed to create case variable for %s\n" CRESET,
                    ctor->name);
            free(case_vars);
            free(case_contexts);
            return NULL;
    ...
    if (index_count > 0) {
        ...
        for (size_t i = 0; i < index_count; i++) {
            ...
            concl_index_vars[i] =
                init_var_expression_wc(index_name, index_type, final_ctx);
    ...
    for (size_t i = 0; i < index_count; i++) {
        ind_applied =
            init_app_expression_wc(ind_applied, concl_index_vars[i], final_ctx);
    }

    Expression *target_var =
        init_var_expression_wc("e", ind_applied, final_ctx);
    ...
    for (size_t i = 0; i < index_count; i++) {
        target_applied = init_app_expression_wc(
            target_applied, concl_index_vars[i], target_ctx);
    }
    target_applied =
...
static Expression *_build_constructor_case_type(
    ...
    for (size_t i = 0; i < arg_count; i++) {
        ...
        if (!ctor_app) {
            fprintf(stderr,
                    ERROR "Failed to apply constructor arg %zu\n" CRESET, i);
            dll_destroy(arg_types);
            free(arg_vars);
            return NULL;
        }
    ...
    if (index_count > 0) {
        ...
        for (size_t i = 0; i < index_count; i++) {
            ctor_indices[i] =
                (Expression *)dll_at(spine, param_count + i)->data;
        }
    ...
    for (size_t i = 0; i < index_count; i++) {
        ...
        if (!case_result) {
            fprintf(stderr,
                    ERROR "Failed to apply motive to index %zu\n" CRESET, i);
            dll_destroy(arg_types);
            free(arg_vars);
            if (ctor_indices) {
                free(ctor_indices);
    ...
    for (size_t i = arg_count; i > 0; i--) {
        case_type =
            init_forall_expression_wc(arg_vars[i - 1], case_type, wrap_ctx);
...
static void _handle_inductive_command(MEngineRuntime *rt,
                                      InductiveCmd *ind_cmd) {
    if (!rt || !ind_cmd) {
        return;
    }
    ...
    for (size_t i = 0; i < param_count; i++) {
        ...
        Expression *param_var =
            init_var_expression_wc(params[i]->name, param_type, c);
        c = context_insert(c, param_var);
    ...
    if (ind_principle_type) {
        Expression *ind_principle_var = init_var_expression_wc(
