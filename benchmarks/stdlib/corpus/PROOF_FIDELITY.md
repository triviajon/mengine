...
### `map_map` → `map_map`  *(proof: untranslatable)*

- **stdlib** — `user-contrib/Stdlib/Lists/List.v:1347` (`Stdlib.Lists.List`):
  ```coq
...
### `rev_unit` → `rev_unit`  *(proof: untranslatable)*

- **stdlib** — `user-contrib/Stdlib/Lists/List.v:991` (`Stdlib.Lists.List`):
  ```coq
    ...
    Qed.
  ```
- **corpus** (`Lists/rocq.v`): `intros A l a. rewrite rev_app_distr; simpl; reflexivity.`
- **why it diverges:** A concrete script exists but relies on automation/structure MEngine's translator does not reproduce mechanically.  (MEngine *does* have `;`, `try`, `repeat`, `first`, `match Goal` and emulated `auto`/`trivial`/`simpl`/`f_equal` — the gap is the leaf tactics above, not sequencing.)  The corpus proves the same statement with MEngine's primitive tactics (`intros A l a. rewrite rev_app_distr; simpl; reflexivity.`).
...
### `sub_succ` → `Nat.sub_succ`  *(proof: functor)*

- **stdlib** — `user-contrib/Stdlib/Numbers/Natural/Abstract/NSub.v:25` (`Stdlib.Numbers.Natural.Abstract.NSub`):
  ```coq
  ...
  ```
- **corpus** (`Nat/rocq.v`): `intros n m. reflexivity.`
- **why it diverges:** `Nat.sub_succ` is produced by module-functor `Include`; its only proof script lives in the abstract functor (`user-contrib/Stdlib/Numbers/Natural/Abstract/NSub.v:25`) over setoid equality `==` (not Leibniz `=`), written with abstract-functor Ltac.  No concrete-`nat` script exists to copy, and MEngine has no module system — the corpus reproves it in MEngine's tactic subset (`intros n m. reflexivity.`).

### `max_0_l` → `Nat.max_0_l`  *(proof: functor)*

- **stdlib** — `user-contrib/Stdlib/Numbers/Natural/Abstract/NMaxMin.v:20` (`Stdlib.Numbers.Natural.Abstract.NMaxMin`):
  ```coq
  ...
  ```
- **corpus** (`Nat/rocq.v`): `intro n. reflexivity.`
- **why it diverges:** `Nat.max_0_l` is produced by module-functor `Include`; its only proof script lives in the abstract functor (`user-contrib/Stdlib/Numbers/Natural/Abstract/NMaxMin.v:20`) over setoid equality `==` (not Leibniz `=`), written with abstract-functor Ltac.  No concrete-`nat` script exists to copy, and MEngine has no module system — the corpus reproves it in MEngine's tactic subset (`intro n. reflexivity.`).

### `max_0_r` → `Nat.max_0_r`  *(proof: functor)*

- **stdlib** — `user-contrib/Stdlib/Numbers/Natural/Abstract/NMaxMin.v:25` (`Stdlib.Numbers.Natural.Abstract.NMaxMin`):
  ```coq
  ...
  ```
- **corpus** (`Nat/rocq.v`): `destruct n. - reflexivity. - reflexivity.`
- **why it diverges:** `Nat.max_0_r` is produced by module-functor `Include`; its only proof script lives in the abstract functor (`user-contrib/Stdlib/Numbers/Natural/Abstract/NMaxMin.v:25`) over setoid equality `==` (not Leibniz `=`), written with abstract-functor Ltac.  No concrete-`nat` script exists to copy, and MEngine has no module system — the corpus reproves it in MEngine's tactic subset (`destruct n. - reflexivity. - reflexivity.`).

### `min_0_l` → `Nat.min_0_l`  *(proof: functor)*

- **stdlib** — `user-contrib/Stdlib/Numbers/Natural/Abstract/NMaxMin.v:30` (`Stdlib.Numbers.Natural.Abstract.NMaxMin`):
  ```coq
  ...
  ```
- **corpus** (`Nat/rocq.v`): `intro n. reflexivity.`
- **why it diverges:** `Nat.min_0_l` is produced by module-functor `Include`; its only proof script lives in the abstract functor (`user-contrib/Stdlib/Numbers/Natural/Abstract/NMaxMin.v:30`) over setoid equality `==` (not Leibniz `=`), written with abstract-functor Ltac.  No concrete-`nat` script exists to copy, and MEngine has no module system — the corpus reproves it in MEngine's tactic subset (`intro n. reflexivity.`).

### `min_0_r` → `Nat.min_0_r`  *(proof: functor)*

- **stdlib** — `user-contrib/Stdlib/Numbers/Natural/Abstract/NMaxMin.v:35` (`Stdlib.Numbers.Natural.Abstract.NMaxMin`):
  ```coq
  ...
  ```
- **corpus** (`Nat/rocq.v`): `destruct n. - reflexivity. - reflexivity.`
- **why it diverges:** `Nat.min_0_r` is produced by module-functor `Include`; its only proof script lives in the abstract functor (`user-contrib/Stdlib/Numbers/Natural/Abstract/NMaxMin.v:35`) over setoid equality `==` (not Leibniz `=`), written with abstract-functor Ltac.  No concrete-`nat` script exists to copy, and MEngine has no module system — the corpus reproves it in MEngine's tactic subset (`destruct n. - reflexivity. - reflexivity.`).

### `max_id` → `Nat.max_id`  *(proof: functor)*

- **stdlib** — `user-contrib/Stdlib/Structures/GenericMinMax.v:153` (`Stdlib.Structures.GenericMinMax`):
  ```coq
  ...
  ```
- **corpus** (`Nat/rocq.v`): `induction n. - reflexivity. - simpl; rewrite IHn; reflexivity.`
- **why it diverges:** `Nat.max_id` is produced by module-functor `Include`; its only proof script lives in the abstract functor (`user-contrib/Stdlib/Structures/GenericMinMax.v:153`) over setoid equality `==` (not Leibniz `=`), written with abstract-functor Ltac.  No concrete-`nat` script exists to copy, and MEngine has no module system — the corpus reproves it in MEngine's tactic subset (`induction n. - reflexivity. - simpl; rewrite IHn; reflexivity.`).

### `min_id` → `Nat.min_id`  *(proof: functor)*

- **stdlib** — `user-contrib/Stdlib/Structures/GenericMinMax.v:296` (`Stdlib.Structures.GenericMinMax`):
  ```coq
  ...
  ```
- **corpus** (`Nat/rocq.v`): `induction n. - reflexivity. - simpl; rewrite IHn; reflexivity.`
- **why it diverges:** `Nat.min_id` is produced by module-functor `Include`; its only proof script lives in the abstract functor (`user-contrib/Stdlib/Structures/GenericMinMax.v:296`) over setoid equality `==` (not Leibniz `=`), written with abstract-functor Ltac.  No concrete-`nat` script exists to copy, and MEngine has no module system — the corpus reproves it in MEngine's tactic subset (`induction n. - reflexivity. - simpl; rewrite IHn; reflexivity.`).
