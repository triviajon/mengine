...
WEAKER = [
    (r"\bf_equal\b", "`f_equal` (the compat prelude now provides a single-layer "
                     "approximation built on `Bad_App_Congruence`; Rocq's is a "
                     "general recursive congruence)"),
...
def _note(category, ref, detail, corpus_proof, original_reason):
    ...
    if category == "untranslatable":
        ...
        if absent:
            ...
            why = (f"uses {', '.join(weaker)} — which MEngine only approximates / "
                   f"emulates more weakly than Rocq, so it cannot replay the proof "
                   f"verbatim")
        else:
        ...
        return (f"A concrete script exists but {why}.  (MEngine *does* have `;`, "
                ...
                f"`auto`/`trivial`/`simpl`/`f_equal` — the gap is the leaf tactics "
                f"above, not sequencing.)  The corpus proves the same statement "
                f"with MEngine's primitive tactics (`{corpus_proof}`).")
