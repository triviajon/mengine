...
class SymbolicExecution(Benchmark):
    ...
    @property
    def params(self):
        return [ParamSpec("n", start=1, stop=76, step=5)]
...
"""
    ...
    def _generate_coq(self, n, workdir):
        content = r"""Require Import Coq.Setoids.Setoid.
...
Section A.
    ...
    let step :=
        ...
        | rewrite partial_map_get_put_diff by apply not_eq_string_b_a
    ...
    Ltac exec_once := match goal with
        ...
        | [ |- exec (cmd_set _ _) _ _ _ _ ] => eapply exec_set; first [
            match goal with
            | [ |- @eq (option word) (eval_expr _ _ _) (option_some word _) ] => solve_eq_by_rewriting
            end
          | idtac
          ]
    ...
    Goal let n := """ + str(n) + r""" in let t := (list_nil IOEvent) in
