...
def _generate_mengine_let_bindings(n):
    """Generate nested let-bindings for mengine using h x x -> x pattern.
    This mirrors rewrite_expo.me: build nested self-applications via let-bindings,
    then rewrite them all away. Each level doubles the term size under the hood.
    """
    lines = []
    for i in range(1, n + 1):
        vprev = "a" if i == 1 else f"v{i-1}"
        lines.append(f"   let v{i}: A := (h {vprev} {vprev}) in")
    last = f"v{n}" if n > 0 else "a"
    lines.append(f"   (h {last} {last})")
    return "\n".join(lines)
...
class Addr0LetIn(Benchmark):
    ...
    def generate(self, strategy, params, workdir):
        ...
        if strategy.engine == "mengine":
            ...
            content = f"""Axiom A : Type.
Axiom h : forall (_: A), forall (_: A), A.
Axiom a : A.
Lemma hxx_x : forall (x: A), eq A (h x x) x.
Admitted.
