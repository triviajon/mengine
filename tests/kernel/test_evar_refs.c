...
void test_fill_hole_cyclic_rejected(void) {
    ...
    assert_true(kernel_expr_has_holes(hole), "hole should have holes (itself)");

    Expression *x = kernel_var_create("x", A, A);
    Expression *app = kernel_app_create(hole, x, x);
    Expression *lam = kernel_lambda_create(x, app);

    assert_true(kernel_expr_has_holes(lam), "lambda containing hole should have holes");
...
void test_evar_refs_propagation(void) {
    ...
    assert_false(kernel_expr_has_holes(id), "id (a variable) should not have holes");
}
...
void test_evar_refs_propagate_from_types(void) {
    ...
    assert_true(kernel_expr_has_holes(x), "variable with hole type should have holes");
}
...
void test_fill_hole_clears_evar_refs(void) {
    ...
    assert_false(kernel_expr_has_holes(lam), "lambda should be evar-free after filling hole");
}
