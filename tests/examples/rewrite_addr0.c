...
RewriteProof *rewrite_addr0__letin(int n_depth) {
    ...
    for (int i = n_depth - 1; i > 0; i--) {
        ...
        curr_expr = init_lambda_expression(
            vi, init_app_expression(init_app_expression(LetIn, subexpr),
                                    curr_expr));
    }
    ...
    Expression *subexpr = init_app_expression(
        init_app_expression(
            add, init_app_expression(init_app_expression(add, v0), v0)),
        O);
    curr_expr =
...
RewriteProof *rewrite_addr0__native(int n_depth) {
    ...
    Expression *expr = init_app_expression(
        init_app_expression(
            add, init_app_expression(init_app_expression(add, vnm1), vnm1)),
        O);
