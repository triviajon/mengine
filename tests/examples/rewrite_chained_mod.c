...
RewriteProof *rewrite_chained_mod(int n_depth) {
    ...
    // Axiomize Lemma mod_mod : forall a n : nat, eq nat (mod (mod a n) n) (mod
    // a n) Need to build the context which the lemma's type is valid under.
    ...
    Expression *subexpr1 =
        init_app_expression(init_app_expression(mod, subexpr0), n);
    Expression *mod_mod_ty = init_forall_expression(
        a,
        init_forall_expression(
            n, init_app_expression(
                   init_app_expression(init_app_expression(eq, nat), subexpr1),
                   subexpr0)));
